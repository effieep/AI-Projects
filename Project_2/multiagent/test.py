    ...
    def evaluationFunction(self, currentGameState: GameState, action):
        ...
        for i, ghostState in enumerate(newGhostStates):
            ...
            if newScaredTimes[i] > 0:
                ...
                # Discourage moving towards unscared ghosts, because they can kill Pacman
        ...
        # If there are capsules, encourage moving towards them
        ...
        score -= len(newFood) * 5  # A constant penalty for each remaining food pellet
...
class MinimaxAgent(MultiAgentSearchAgent):
    ...
    def min_value(self, gameState: GameState, depth: int, a = 0):
        ...
        for action in gameState.getLegalActions(a):
            ...
            value = 0
            # if a == 0 it is the turn of pacman
...
class AlphaBetaAgent(MultiAgentSearchAgent):
    ...
    def min_value_ab(self, gameState: GameState, depth: int, alpha: float, beta: float, a = 0):
        ...
        for action in gameState.getLegalActions(a):
            ...
            # if a == 0 it is the turn of pacman
            if a == gameState.getNumAgents() - 1:  
                value = self.max_value_ab(successor, depth + 1)[0]
            # if it is the turn of a ghost, get the min value from the next ghost
            else:
                # if it is the turn of a ghost, get the min value from the next ghost
                value = self.min_value_ab(successor, depth, a + 1)[0]
    ...
    def max_value_ab(self, gameState: GameState, depth: int, alpha: float, beta: float, a = 0):
        ...
        for action in gameState.getLegalActions(a):
            successor = gameState.generateSuccessor(a, action)
            value = self.min_value_ab(successor, depth, 1)[0]
            if value > maxv:
                maxv = value
                max_action = action
            if value > beta:
