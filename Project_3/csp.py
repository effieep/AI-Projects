...
def weight(csp, var, assignment):
    if csp.curr_domains:
        return len(csp.curr_domains[var])/csp.weights[var]
    else:
        return count(csp.nconflicts(var, val, assignment) == 0 for val in csp.domains[var])/csp.weights[var]

def dom_wdeg(assignment, csp):
    """Minimum-remaining-values heuristic."""
    return argmin_random_tie([v for v in csp.variables if v not in assignment],
                             key=lambda var: weight(csp, var, assignment))
